...
from modules.whisper import AudioEncoder
...
@dataclass
class WhisperInputBatch:
	''' A batch of training data for the Whisper model. '''
	x_src: torch.Tensor
	x_tgt: torch.Tensor
...
class Whisper(pl.LightningModule):
	...
	def __init__(self, config: WhisperConfig, tokenizer: Tokenizer):
		...
		self.example_input_array = (torch.zeros(1, config.enc_max_len, 80, dtype=torch.float),
			      					torch.zeros(1, config.dec_max_len, dtype=torch.long),
								    # actual input length to the encoder transformer is half the max length as the CNN downsamples by a factor of 2
									torch.zeros(1, config.enc_max_len // 2, dtype=torch.long),
									torch.zeros(1, config.dec_max_len, dtype=torch.long))
		# model parts
	...
	def encoder_forward(self, src: Tensor, src_tok_mask: Tensor):
		...
		mel_pos_embedding = self.mel_pos_embedding(torch.tensor(features.size(1), dtype=torch.long, device=src.device))
		features = mel_pos_embedding + features
		return self.encoder(features, src_tok_mask)
	...
	def forward(self, src: Tensor, tgt: Tensor, src_tok_mask: Tensor, tgt_tok_mask: Tensor):
		...
		mel_pos_embedding = self.mel_pos_embedding(torch.tensor(features.size(1), dtype=torch.long, device=src.device))
		features = mel_pos_embedding + features
		enc = self.encoder(features, src_tok_mask)
		dec = self.decoder(enc, self.transcript_embeddings(tgt), src_tok_mask, tgt_tok_mask)
		logits = self.lm_head(dec)
		return logits
	...
	def validation_step(self, batch: WhisperInputBatch, batch_idx: int):
		y_pred = self(batch.x_src, batch.x_tgt, batch.x_src_mask, batch.x_tgt_mask)
	...
	def test_step(self, batch: WhisperInputBatch, batch_idx: int):
		y_pred = self(batch.x_src, batch.x_tgt, batch.x_src_mask, batch.x_tgt_mask)
